...
const Login = () => {
  ...
  return (
    <div className="flex-grow flex items-center justify-center bg-gradient-to-br from-cyan-400 via-teal-300 to-yellow-200 p-4">
      {/* Glassmorphism Card */}
      <div className="w-full max-w-md bg-black/20 backdrop-blur-lg border border-white/20 p-8 rounded-2xl shadow-2xl">
        ...
        {authMode === 'email' && (
          <form onSubmit={handleEmailLogin} className="space-y-4">
            ...
            <button 
              disabled={loading}
              className="w-full bg-white/20 hover:bg-white/30 text-white font-bold py-3 rounded-xl transition-all shadow-lg flex justify-center items-center h-12"
            >
        ...
        {authMode === 'phone' && (
          <form onSubmit={handleSendOtp} className="space-y-4">
            <div>
              <label className="block text-white/80 text-sm font-medium mb-1">Phone Number (with country code)</label>
              <div className="relative">
        ...
        {authMode === 'otp' && (
          <form onSubmit={handleVerifyOtp} className="space-y-4">
            <div>
              <label className="block text-white/80 text-sm font-medium mb-1">Enter OTP Code</label>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-white/50" />
                <input 
                  type="text" 
                  required 
                  className="w-full bg-white/5 border border-white/10 rounded-xl py-3 pl-10 pr-4 text-white placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-white/50 transition-all tracking-widest text-center text-lg"
                  placeholder="000000"
                  value={otp}
        ...
        {/* Alternate auth toggles */}
        {!['otp'].includes(authMode) && (
          <div className="mt-6 flex flex-col gap-3">
            ...
            <button 
              ...
              className="w-full bg-transparent border border-white/30 hover:bg-white/10 text-white font-medium py-3 rounded-xl transition-all"
            >
              {authMode === 'email' ? 'Login with Phone Number' : 'Login with Email'}
            </button>
          </div>
        )}
        ...
        {/* Invisible exact container for reCaptcha rendering */}
